				...
				<!-- Header -->
					<header id="header" class="alt">
						...
						<nav>
							...
							<c:if test="${sMember.email eq 'admin'}">
						...
						<div class="inner">
							<header class="major">
								<!-- 로그인 후 로그인 한 사용자의 세션아이디로 바꾸시오.
									 ex)smart님 환영합니다 
									 session에 데이터가 있는 경우, 출력
									 session에 데이터가 없는 경우, h1태그만 출력
									 -->
									 <c:if test="${!empty sMember}">
									 	<h1>${sMember.email}님 환영합니다.</h1>
									 </c:if>
									 <c:if test="${empty sMember}">
